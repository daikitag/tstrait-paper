...
%DIF DEL original-paper.tex   Mon May 13 18:26:18 2024
%DIF ADD paper.tex            Mon May 13 18:27:27 2024
...
\DIFaddbegin \DIFadd{Dominance and epistasis are straightforward extensions to this
model, which we plan to include in future versions of }\texttt{\DIFadd{tstrait}}\DIFadd{.
}\DIFaddend 
...
processed using standard Python data science tools\DIFaddbegin \DIFadd{, or exported
to (e.g.) CSV for broad compatibility}\DIFaddend .
...
%DIFDELCMD < \centering
%DIFDELCMD < %%%
%DIF <  FIXME get the sizes right here and in the notebook where we're generating
%DIF <  the figure so we don't have to scale here.
%DIFDELCMD < \includegraphics[width=\columnwidth]{figures/stdpopsim-time.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Time taken to simulate quantitative traits with increasing
sample size. For each sample size, we simulated an ARG under human-like
parameters using the default HomSap demographic model in }\texttt{\DIFdelFL{stdpopsim}}%DIFAUXCMD
\DIFdelFL{.
Each point represents the mean time for 10 independent runs of }\texttt{\DIFdelFL{tstrait}}
%DIFAUXCMD
\DIFdelFL{for a particular ARG. The times reported are the total CPU
