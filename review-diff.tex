...
%DIF DEL paper.tex            Mon May 13 18:27:27 2024
%DIF ADD original-paper.tex   Mon May 13 18:26:18 2024
...
\DIFdelbegin \DIFdel{Dominance and epistasis are straightforward extensions to this
model, which we plan to include in future versions of }\texttt{\DIFdel{tstrait}}%DIFAUXCMD
\DIFdel{.
}\DIFdelend 
...
processed using standard Python data science tools\DIFdelbegin \DIFdel{, or exported
to (e.g.) CSV for broad compatibility}\DIFdelend .
...
\caption{\DIFaddFL{Time taken to simulate quantitative traits with increasing
sample size. For each sample size, we simulated an ARG under human-like
parameters using the default HomSap demographic model in }\texttt{\DIFaddFL{stdpopsim}}\DIFaddFL{.
Each point represents the mean time for 10 independent runs of }\texttt{\DIFaddFL{tstrait}}
\DIFaddFL{for a particular ARG. The times reported are the total CPU
